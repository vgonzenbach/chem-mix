...
nt_df <- read.csv(here::here('data/neurotransmitter_interaction.csv'), row.names = NULL) %>% 
  ...
  pivot_longer(cols = c("mI", "NAA", "Cr", "Cho", "Glu", "Glx"), 
               names_to = 'variable', 
               values_to = 'p.values') %>% 
  tibble::add_column(var.type = 'Brain Metabolite', .after  = 'variable') %>% 
  filter(p.values < 0.1) %>% 
...
The first network visualization summarizes the main effects of chemical mixtures (in red arrows).

Window-specific networks are then presented, with gray arrows representing the presence of a significant interaction term (i.e. time window-specific effect), and with main effects being projected again as red arrows. 
...
vizNetwork <- function(vis.nodes, vis.links){
  ...
  if (all(unique(vis.links$color) == 'red')){
    visnet <- visnet %>% 
...
(main_df <- df %>% 
  ...
  rename(outcome = variable,
         effect = term) %>% 
  unite(outcome, outcome, var.type, sep = ' ') %>% 
  relocate(effect, .before = 'OPE'))
```
...
```{r}
rbind(main_df, prenatal_df, birth_df, three_year_df) %>% 
  count(OPE, name = 'number_of_effects') %>% 
  arrange(desc(number_of_effects))
```
